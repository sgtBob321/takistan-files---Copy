...
if(!(life_gangData isEqualTo [-1,-1]) OR _bool) then {
    ...
    (_display displayCtrl 220051) ctrlSetStructuredText parseText format[localize "STR_aapps_gang_stats",_tag,_group getVariable ["gang_name","1337 BUGGY 1337"],[_group getVariable ["gang_bank",0]] call life_fnc_numbertext,count _members,if(_type isEqualTo 0) then {getNumber(missionConfigFile >> "gang_config" >> "default_temp_maxmembers")} else {getNumber(missionconfigfile >> "gang_config" >> "upgrades" >> format["level%1",_lvl] >> "max_members")},_lvl,_tax,"%", [_sum] call life_fnc_numbertext];
    ...
    _val = 0;
    {
        ...
        if(_bool isEqualTo 1) then {
            if(!((_x isEqualTo "DEPOSIT") && {"WITHDRAW" in _playerpermission})) then {
    ...
    if(_bool && {!(_permission isEqualTo 0)}) then {
        _listbox lbAdd localize "STR_aapps_gang_action_reassign";
        _listbox lbSetData [_val,"0 call life_fnc_gang_reassign"];
        //_listbox lbSetPicture [_val,""];
        _val = _val + 1;
    };
    if((_permission isEqualTo 0) OR {"TAG" in _playerpermission} OR {"TAX" in _playerpermission}) then {
