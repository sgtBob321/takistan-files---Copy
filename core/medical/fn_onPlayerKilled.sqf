...
if(!(FETCH_CONST(life_adminlevel) > 1)) then {(findDisplay 7300) displaySetEventHandler ["KeyDown","if((_this select 1) == 1) then {true}"]; } else {};//Block the ESC menu
...
if(isNull _killer || _killer == player) then {
	_Killedby ctrlSetText format ["Killed by: %1 (%2) (%3)",_unit getVariable["realname",""],_groupName, getPlayerUID _unit];
} else {
	_Killedby ctrlSetText format ["Killed by: %1 (%2) (%3)",_killer getVariable["realname",""],_groupNameKiller, getPlayerUID _killer];
};
...
if (!isNull _killer && {!(_killer isEqualTo _unit)}) then {
    [player getVariable ["realname",name player]] remoteExec ["life_fnc_killfeed",_killer];
};
