...
_kassa = 30000 + round(random 30000);
...
_pgText ctrlSetText format["Robbing, clear your inventory for goldcoins, stay within 25 meters (1%1)...","%"];
...
while{true} do
{
	...
	if!(alive _robber) exitWith {  life_rip = false; call life_fnc_hudUpdate; 5 cutText ["","PLAIN"];  };
...
if!(alive _robber) exitWith {};
[0,format["Police News: The bank has just been robbed: the stolen amount of money was $%3 and some gold bars!",name _robber, _shop, [_kassa] call life_fnc_numberText]] remoteExec ["life_fnc_broadcast",west];
[0,format["Police News: The bank has just been robbed: the stolen amount of money was $%3 and some gold bars, if you have seen the robbers, contact police!",name _robber, _shop, [_kassa] call life_fnc_numberText]] remoteExec ["life_fnc_broadcast",civilian];
[getPlayerUID _robber,name _robber,"15"] remoteExec ["life_fnc_wantedAdd",2];

[] call life_fnc_hudUpdate;
uisleep 10;
_action = _shop addAction["Rob Bank",life_fnc_robBank];
