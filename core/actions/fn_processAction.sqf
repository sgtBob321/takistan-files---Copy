...
if (_exit) exitWith {life_is_processing = false; [localize "STR_NOTF_NotEnoughItemProcess",true,"fast"] call life_fnc_notificationSystem; life_action_inUse = false;}; //hint localize "STR_NOTF_NotEnoughItemProcess"
...
if (_exit) exitWith {[localize "STR_Process_Weight",true,"fast"] call life_fnc_notificationSystem; life_is_processing = false; life_action_inUse = false;}; //hint localize "STR_Process_Weight";
...
_cP = 0.01;
private _notifProcessOne = format [localize "STR_Process_License",[_cost] call life_fnc_numberText];
...
if (_hasLicense) then {
    ...
    if (player distance _vendor > 10) exitWith {[localize "STR_Process_Stay",true,"fast"] call life_fnc_notificationSystem; "progressBar" cutText ["","PLAIN"]; life_is_processing = false; life_action_inUse = false;}; //hint localize "STR_Process_Stay";
    ...
    if (_minimumConversions isEqualTo (_totalConversions call BIS_fnc_lowestNum)) then {[localize "STR_NOTF_ItemProcess",false,"fast"] call life_fnc_notificationSystem;} else {[localize "STR_Process_Partial",false,"fast"] call life_fnc_notificationSystem;};//hint localize "STR_NOTF_ItemProcess"; hint localize "STR_Process_Partial";
    life_is_processing = false; life_action_inUse = false;
    ["ItemProcessed"] spawn mav_ttm_fnc_addExp;
} else {
    if (CASH < _cost) exitWith {[_notifProcessOne,true,"fast"] call life_fnc_notificationSystem; "progressBar" cutText ["","PLAIN"]; life_is_processing = false; life_action_inUse = false;};//hint format [localize "STR_Process_License",[_cost] call life_fnc_numberText];
